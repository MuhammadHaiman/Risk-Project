    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" class="w-full px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ $user->name }}" required>
            </div>

            <div class="mb-4">
                <label for="email" class="block text-sm font-semibold text-gray-700 mb-2">Email</label>
                <input type="email" name="email" id="email" class="w-full px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent" value="{{ $user->email }}" required>
            </div>

            <div class="mb-4">
                <label for="password" class="block text-sm font-semibold text-gray-700 mb-2">Kata Laluan (Kosongkan jika tidak diubah)</label>
                <input type="password" name="password" id="password" class="w-full px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="role" id="role" class="w-full px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent" required onchange="toggleAgencyField()">
                    ...
                    <option value="agency" {{ $user->role === 'agency' ? 'selected' : '' }}>Agensi</option>
                </select>
            </div>
            ...
            <div class="mb-4" id="agencyField" style="display: none;">
                ...
                <select name="agensi_id" id="agensi_id" class="w-full px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    @foreach ($agencies as $agency)
                        <option value="{{ $agency->id }}" {{ $user->agensi_id == $agency->id ? 'selected' : '' }}>{{ $agency->nama_agensi }}</option>
                    @endforeach
                </select>
            </div>
