...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-lg shadow p-8 max-w-2xl">
            <form action="{{ route('agency.risks.update', $risk->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <select name="aset_id" id="aset_id"
                        ...
                        @foreach ($assets as $asset)
                            <option value="{{ $asset->id }}" @selected($risk->aset_id == $asset->id)>{{ $asset->nama_aset }}
                            </option>
                        @endforeach
                    </select>
                    @error('aset_id')
                ...
                <div class="mb-6 p-4 bg-blue-50 rounded border border-blue-200">
                    ...
                    <select name="kategori_id" id="kategori_id"
                        ...
                        @foreach ($kategoris as $kategori)
                            <option value="{{ $kategori->id }}" @selected($risk->kategori_id == $kategori->id)>{{ $kategori->nama }}</option>
                        @endforeach
                    </select>
                    @error('kategori_id')
                        <span class="text-red-600 text-sm">{{ $message }}</span>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const currentKategori = '{{ $risk->kategori_id }}';
            const currentSubKategori = '{{ $risk->sub_kategori_risiko_id }}';
            const currentRisiko = '{{ $risk->risiko_id }}';
            const currentPunca = '{{ $risk->punca_risiko_id }}';
                        ...
                        .then(data => {
                            if (data.length > 0) {
                                ...
                                data.forEach(item => {
                                    ...
                                    if (item.id == currentSubKategori) {
                                        option.selected = true;
                                    }
                                    subKategoriSelect.appendChild(option);
                                });

                                // Trigger change to load risks
