...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6 bg-white rounded-lg shadow p-6">
            <form method="GET" action="{{ route('agency.reports.highest-risk') }}"
        ...
        <div class="mb-6 bg-white rounded-lg shadow p-6">
            <div class="flex gap-2">
                <a href="{{ route('agency.reports.highest-risk-pdf') }}?{{ request()->getQueryString() }}"
                    class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 flex items-center gap-2">
                    <i class="fas fa-file-pdf"></i> Export PDF
                </a>
                <a href="{{ route('agency.reports.highest-risk-excel') }}?{{ request()->getQueryString() }}"
            ...
            <div class="mb-8 bg-white rounded-lg shadow overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody>
                            @forelse ($data['risks'] as $index => $item)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-3 text-sm text-gray-700 font-semibold">{{ $item->risiko->nama }}
                                    </td>
                                    <td class="px-6 py-3 text-sm text-gray-700">
                                        {{ $item->risiko->subKategoriRisiko->nama }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-center text-gray-600">Tiada data risiko
                                        tersedia</td>
                                </tr>
                            @endforelse
                        </tbody>
